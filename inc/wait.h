...
#define number_of_cycles_ms(ms, hz)                                            \
  ((ms) *                                                                      \
   ((hz) /                                                                     \
    1000)) /* calculates the needed cycles based on bus clock frequency */
...
#define WAIT_C(cycles)                                                         \
  ((cycles) <= 10 ? wait_10cycles()                                            \
                  : wait_cycles(cycles)) /*!< wait for some cycles */
