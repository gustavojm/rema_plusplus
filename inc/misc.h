...
void stb_linear_controller(float *curpos, float target_pos, float acc,
                           float deacc, float dt) {
...
double stb_linear_remap(double x, double x_min, double x_max, double out_min,
                        double out_max) {
...
#define stb_little32(c)                                                        \
  (((c)[3] << 24) + (c)[2] * 65536 + (c)[1] * 256 + (c)[0])
...
int stb_bitcount(
    unsigned int
        a) { // http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
...
int stb_is_pow2(size_t n) { return (n & (n - 1)) == 0; }
