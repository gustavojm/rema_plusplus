...
public:
  ...
  void reply_fn(int sock) override {
    ...
    while (true) {
      ...
      ans["telemetry"]["targets"]["x"] =
          x_y_axes->first_axis->destination_counts /
          static_cast<double>(x_y_axes->first_axis->inches_to_counts_factor);
      ans["telemetry"]["targets"]["y"] =
          x_y_axes->second_axis->destination_counts /
          static_cast<double>(x_y_axes->second_axis->inches_to_counts_factor);
      ans["telemetry"]["targets"]["z"] =
          z_dummy_axes->first_axis->destination_counts /
          static_cast<double>(
              z_dummy_axes->first_axis->inches_to_counts_factor);

      struct limits limits = encoders->read_limits();

      ans["telemetry"]["limits"]["left"] =
          static_cast<bool>(limits.hard & 1 << 0);
      ans["telemetry"]["limits"]["right"] =
          static_cast<bool>(limits.hard & 1 << 1);
      ans["telemetry"]["limits"]["up"] =
          static_cast<bool>(limits.hard & 1 << 2);
      ans["telemetry"]["limits"]["down"] =
          static_cast<bool>(limits.hard & 1 << 3);
      ans["telemetry"]["limits"]["in"] =
          static_cast<bool>(limits.hard & 1 << 4);
      ans["telemetry"]["limits"]["out"] =
          static_cast<bool>(limits.hard & 1 << 5);
      ans["telemetry"]["limits"]["probe"] =
          static_cast<bool>(limits.hard & 1 << 6);
      ...
      ans["telemetry"]["probe_protected"] =
          x_y_axes->was_stopped_by_probe_protection ||
          z_dummy_axes->was_stopped_by_probe_protection;
      ...
      size_t msg_len =
          json::serializeMsgPack(ans, tx_buffer, sizeof(tx_buffer) - 1);
