...
mot_pap::mot_pap(const char *name) :
        name(name), type(MOT_PAP_TYPE_STOP), last_dir(MOT_PAP_DIRECTION_CW),
                half_pulses(0), offset(0) {
}
...
uint16_t mot_pap::offset_correction(uint16_t pos, uint16_t offset) {
    ...
    if (corrected < 0)
        corrected = corrected + (int32_t) (1 << rdc->get_resolution());
...
void mot_pap::new_cmd_received() {
    stalled = false;  // Assume we are not stalled
    stalled_counter = 0;
    already_there = false;
...
void mot_pap::move_free_run(enum direction direction, uint32_t speed) {
    if (free_run_speed_ok(speed)) {
        ...
        dir = direction;
        gpios.direction(dir);
        freq = free_run_freqs[speed] * 1000;
