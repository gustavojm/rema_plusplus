...
JSON_Value *tcp_server_command::mem_info_cmd(JSON_Value const *pars) {
  ...
  json_object_set_number(root_object, "MEM_MIN_FREE",
 ...
 * function
 */
JSON_Value *tcp_server_command::cmd_execute(char const *cmd,
...
int tcp_server_command::json_wp(char *rx_buff, char **tx_buff) {
  ...
  *tx_buff = NULL;
  ...
  if (!rx_JSON_value || (json_value_get_type(rx_JSON_value) != JSONObject)) {
    ...
    JSON_Array *commands = json_object_get_array(rx_JSON_object, "commands");
