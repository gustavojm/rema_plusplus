...
tcp_server::tcp_server(const char *name, int port) : name(name), port(port) {
  ...
  xTaskCreate([](void *me) { static_cast<tcp_server *>(me)->task(); },
              task_name, 1024, this, configMAX_PRIORITIES, NULL);
...
void tcp_server::task() {
  ...
  int err =
      lwip_bind(listen_sock, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
  if (err != 0) {
  ...
  while (1) {
    ...
    int sock =
        lwip_accept(listen_sock, (struct sockaddr *)&source_addr, &addr_len);
    if (sock < 0) {
