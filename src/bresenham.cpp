...
void bresenham::calculate() {
  first_axis->delta =
      abs(first_axis->destination_counts - first_axis->current_counts);
  second_axis->delta =
      abs(second_axis->destination_counts - second_axis->current_counts);
...
void bresenham::move(int first_axis_setpoint, int second_axis_setpoint) {  
  ...
  first_axis_setpoint =
      std::clamp(first_axis_setpoint, (static_cast<int>(INT32_MIN) / 2),
  ...
  lDebug(Info, "MOVE, %c: %i, %c: %i", first_axis->name, first_axis_setpoint,
         second_axis->name, second_axis_setpoint);
...
void bresenham::isr() {
  ...
  already_there =
      first_axis->check_already_there() && second_axis->check_already_there();
  if (already_there) {
    stop();
