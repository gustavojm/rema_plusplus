							 ...
							 struct pbuf *p)
{
	...
	LWIP_DEBUGF(EMAC_DEBUG | LWIP_DBG_TRACE,
				("lpc_rxqueue_pbuf: Queueing packet %p at index %li, free %li\n",
...
static err_t lpc_low_level_output(struct netif *netif, struct pbuf *sendp)
{
	...
	while (dn > 0) {
		...
		LWIP_DEBUGF(EMAC_DEBUG | LWIP_DBG_TRACE,
					("lpc_low_level_output: pbuf packet %p sent, chain %li,"
					 " size %d, index %li, free %ld\n", p, dn, p->len, idx,
					 lpc_netifdata->tx_free_descs));
...
s32_t lpc_rx_queue(struct netif *netif)
{
	...
	while (lpc_netifdata->rx_free_descs > 0) {
		...
		if (p == NULL) {
			LWIP_DEBUGF(EMAC_DEBUG | LWIP_DBG_TRACE,
						("lpc_rx_queue: could not allocate RX pbuf index %li, "
...
void lpc_tx_reclaim(struct netif *netif)
{
	...
	while ((lpc_netifdata->tx_free_descs < LPC_NUM_BUFF_TXDESCS) &&
		...
		LWIP_DEBUGF(EMAC_DEBUG | LWIP_DBG_TRACE,
					("lpc_tx_reclaim: Reclaiming sent packet %p, index %li\n",
